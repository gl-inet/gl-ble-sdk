/*****************************************************************************
 * @file  demo_bleAdvLegacy.c
 * @brief Enable BLE Legacy broadcast sending function
 *******************************************************************************
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2)
	{
		...
		{
			data0 = json_object_get_string(legacy_adv_data_o);
		}

		json_object *legacy_rsp_data_o = json_object_object_get(file_o, "legacy_rsp_data");
		if(legacy_rsp_data_o != NULL)
		{
			data1 = json_object_get_string(legacy_rsp_data_o);
		}
	...
	if (data0) 
	{
		ret = gl_ble_set_legacy_adv_data(handle, 0, data0);
		if (GL_SUCCESS != ret)
		{
	...
	if (data1)
	{
		ret = gl_ble_set_legacy_adv_data(handle, 1, data1);
		if (GL_SUCCESS != ret)
		{
			printf("gl_ble_set_legacy_adv_data failed: %d\n", ret);
			exit(-1);
	...
	// start advertising
	ret = gl_ble_start_legacy_adv(handle, interval_min, interval_max, discover, connect);
	if (GL_SUCCESS != ret)
	{
...
static void sigal_hander(int sig)
{
	printf("\bleAdvertiserLegacy exit!\n");
